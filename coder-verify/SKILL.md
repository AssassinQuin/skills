...
phase: 5
name: coder-verify
description: Phase 5 验证 — 3 reviewer 并发模板（正确性 / S.U.P.E.R / 安全）+ fresh-context 对抗式审查 + v7.1 model 显式 + [3,5] 上限。
source: "design.md §5.6 + §6.2 + v7.1 §4 #14"
status: skeleton
tokens_estimate: 1200
load_priority: on-demand
load_when: "进入 Phase 5"
keywords: 3 reviewer concurrent correctness project security test-runner test-plan runner spawn limit model explicit
domain: coding
subdomain: phase
parent_skill: coder
version: "1.1"
...
frameworks:
  ...
  notes: "3 reviewer 并发覆盖 correctness/project/security + v7.1 model 显式"
---
...
| reviewer | agent 文件 | model | 职责 | 关注点 |
|---|---|---|---|---|
| **correctness-reviewer** | `agents/correctness-reviewer.md` | sonnet | 验收 checklist 逐条核对 + edge case | 功能正确性 + 并发 + 错误处理 |
| **project-reviewer** | `agents/project-reviewer.md` | sonnet | S.U.P.E.R 衰减 + 项目惯例一致性 | 模块健康分 + codestyle |
| **security-reviewer** | `agents/security-reviewer.md` | sonnet | 私钥 / SQL 注入 / 权限绕过 / PII | OWASP top 10 |

**3 路并发 fresh-context 审查**（互不可见）。**总数 = 3 ✓**（在 v7.1 §4 #14 [3,5] 区间内）。

## 并发模板（v7.1：每个 spawn 必显式 model）

```yaml
spawn:
  - subagent_type: correctness-reviewer
    model: sonnet         # ← v7.1 必填（R5.1 + §4 #2）
    description: "正确性审查"
    prompt: "${REVIEWER_CORRECTNESS_PROMPT}"
  - subagent_type: project-reviewer
    model: sonnet         # ← S.U.P.E.R / 惯例审查
    description: "S.U.P.E.R + 项目惯例回归"
    prompt: "${REVIEWER_PROJECT_PROMPT}"
  - subagent_type: security-reviewer
    model: sonnet         # ← OWASP 审查
    description: "安全审查"
...
**Model 选择理由**：
- 3 个 reviewer 都是 sonnet（审查是工程性推理，不需 opus 战略）
- **同 model × 不同 agent 文件 × 不同 prompt** = 合规（不是"全部继承主 agent"，是按职责选 agent）
- **禁止** spawn reviewer 时用 opus（浪费 + 慢，opus 该留给 oracle）
- **禁止** spawn reviewer 时省略 `model:`（默认继承主 agent 会让 reviewer 变 opus 或 haiku）
