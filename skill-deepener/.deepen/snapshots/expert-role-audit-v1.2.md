...
禁止给 4 分无证据。4 分必须说明"差什么到 5"。

### 方法论严谨者评分边界

| 分 | 边界 |
|----|------|
| 5 | 方法论清晰可证伪 + 反例齐全 + 5 why 不破 + **Expert 视角段齐全** |
| 4 | 方法论 OK 但**反例不足** OR **缺 expert reasoning** OR 5 why 在某层断 |
| 3 | 有方法但部分模糊（如"按需优化"无具体标准） |
| 2 | 多数指令是抽象动词（处理/优化/分析）无具体步骤 |
| 1 | 零方法论，纯流程列表 |
...
| 分 | 边界 |
|----|------|
| 5 | 案例全真实可追溯 + 失败模式齐全 + 5 字段（trigger/failure/expert/rule/evidence）齐 |
| 4 | 案例真实但**happy path 偏多** OR **缺 1-2 字段** OR 案例数 < 3 |
| 3 | 案例真实但泛泛（"用户问 X，agent 答 X"） |
| 2 | 案例部分编造 / 无失败模式 |
| 1 | 零案例 / 全编造 / 无来源 |
...
| 分 | 边界 |
|----|------|
| 5 | 术语精准 + 符合 expert 共识 + 含领域陷阱 + **WebSearch 验证未过期** |
| 4 | 术语准但**部分过期**（已用 WebSearch 标注）OR 缺领域陷阱 |
| 3 | 术语基本对但**含 1-2 处误用** |
| 2 | 多处术语误用 / 不符合 expert 共识 |
| 1 | 严重误用 / 完全不符 expert 共识 |

### 综合 verdict 决策（v1.1 更新）

| 条件 | verdict |
|------|---------|
| 任一角色 ≤ 2 OR 总分 < 10 | **reject**（回 P4 重写） |
| 任一角色 = 3 OR 总分 10-13 | **revise**（P4 局部改） |
| 全部 ≥ 4 + 总分 14-15 | **accept**（进 P6） |
| 全部 = 5 | **强 accept + 标 ⭐ 高质量** |
