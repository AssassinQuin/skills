...
所有任务类型共用 5 步流程。**步骤 2 必须等用户确认后才可执行。**

| 步骤 | 做什么 | 输出物 | 验证门 |
|------|--------|--------|--------|
| **1. 理解** | gopls 导航：搞清楚改什么、影响谁 | 改动清单 + 影响范围 + 风险评级 | 清单条目 > 0 |
| **2. 方案** | 告诉用户要改什么、为什么 | diff 预览 + 改动说明 + 风险提示 | **用户确认** |
| **3. 执行** | 分步改动，每步验证 | 逐步改动结果 | 每步 `go_diagnostics` 零错误 |
| **4. 验证** | build + test + vet | 通过/失败 + 失败归因 | `go build` 通过 |
...
**高风险操作必须明确标注**：公开 API 变更、跨包移动、数据库相关改动。
