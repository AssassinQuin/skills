...
- `SKILL_LIBRARY_ROOT`：用户的私有 skill 库根目录。默认 `~/.claude/skills`。所有 P0 缓存查询使用此变量，不硬编码绝对路径。
...
2. 历史档案：glob("{SKILL_LIBRARY_ROOT}/skill-search/data/**/*.md") + grep("{功能词}")
...
**长文件决策门（v5.1 强制，硬约束 #14 的执行门）**：

`zread_read_file` 后**立即检查返回的行数**（不能用直觉判断），按以下决策树：

```
返回文件 < 300 行？
├── Yes → 全文进主上下文 OK
└── No → 是否韩文/日文为主？
         ├── Yes + 行数 > 150 → 强制 ctx_index 转换（韩日 token ×1.5-2 效率差）
         ├── No + 行数 > 300 → 强制 ctx_index 转换
         └── No + 行数 > 500 → 强制 ctx_index 转换（即使英文）
...
ctx_index 转换协议：
  ...
  3. 主上下文只接收 ctx_search 返回的关键段，不接收全文
```

**反例**（典型场景）：某候选的 references/ 文件 > 7000 tokens 全文进主上下文（韩文/日文文档 token 效率差尤其严重），违反硬约束 #14。v5.1 强制此决策门后，应该 ctx_index 后只取关键段（~1500 tokens）。

**Token 策略**（长文档保护，与决策门配套）：
- SKILL.md > 300 行 → 强制走"长文件决策门"
- Issue body > 2000 字 → 用 `ctx_execute_file` 过滤标题+label+前 500 字
- 韩文/日文文档（token 效率差）→ 同上，且阈值降低到 150 行
