...
如果连续 2 次 `ToolSearch select:Task` 后调用 `Task()` 仍报错，触发 L2 fallback。

## Agent Template：启动子 Agent

`Task()` 是 Claude Code 中启动子 Agent 的标准工具。三个核心参数：

```
Task(
  subagent_type="scout-gh",      # 必需：子 Agent 类型（需在 ~/.claude/agents/ 下注册）
...
必须在**单个 tool_calls 块**中并行调用（5 个 Task 同时发出）：

```
# 在一个 message 中并行调用
Task(subagent_type="scout-gh", model="sonnet", description="...", prompt="...")
Task(subagent_type="scout-builtin", model="sonnet", description="...", prompt="...")
Task(subagent_type="scout-market", model="sonnet", description="...", prompt="...")
Task(subagent_type="scout-community", model="sonnet", description="...", prompt="...")
Task(subagent_type="scout-expand", model="sonnet", description="...", prompt="...")
```
...
触发条件：`ToolSearch select:Task` 后 `Task()` 报错，或连续 2 次返回空 schema。
