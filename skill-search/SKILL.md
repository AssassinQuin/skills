...
**注意**：`Agent` 是 Claude Code 启动子 Agent 的工具（**不是 `TaskCreate`** — TaskCreate 只是任务列表管理）。ToolSearch `select:` 命中后即使返回空，工具也已注册，直接调用即可。
...
Scout 子 Agent 的 prompt 模板在 `agents/scout-*.md`，主 agent 调用时按需注入 {功能词}/{同义词} 等参数。修改 Scout 行为只改 `agents/scout-*.md`，不动 SKILL.md。
...
5 个子 Agent（model: sonnet）并行，prompt 模板见 `agents/scout-*.md`，搜索词模板见 [search-templates.md](references/search-templates.md)：

| 子 Agent | 搜索源 | 核心规则 |
|----------|--------|---------|
| Scout-GH | GitHub | **只用 `search_repositories`**，禁止 `search_code` |
| Scout-BuiltIn | 本地 + Marketplace | 三层全执行，不允许跳过 |
...
3. **Agent() 调用必须传 subagent_type + model** — 启动子 Agent 用 `Agent` 工具（不是 `TaskCreate`）；model 选择：搜索用 sonnet，分析评估用 opus；subagent_type 用 Claude Code 内置（`general-purpose`/`Explore`）或全局 `~/.claude/agents/` 已注册的；skill-local `agents/*.md` 作为 prompt 模板
