...
根据用户需求，多源搜索候选 Skill，按自检清单驱动的流程筛选评估，输出结构化推荐报告。
...
5 个子 Agent（model: sonnet）并行，搜索模板见 [search-templates.md](references/search-templates.md)：
...
5. **子 Agent 必须指定 model** — 搜索用 sonnet，分析评估用 opus
