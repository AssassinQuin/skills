...
load_when: "Phase 6 写 memory"
keywords: memory MCP tier project shared global seed tags convention trap gotcha
domain: coding
subdomain: strategy
parent_skill: coder
version: "1.0"
...
frameworks:
  ...
  notes: "3-tier isolation（project/shared/global）"
---
...
- 输入：`snapshots/references-legacy-v3.2/{lang}-*.md`（v3.2 文件备份）
...
v3.2 references 移到 `snapshots/references-legacy-v3.2/`（保留历史，不进加载链）。
seed 完成后可删 `snapshots/references-legacy-v3.2/`（用户决定）。
