...
💡灵感主题：主动搜索反常识、跨领域连接、被忽视的声音。在其他主题搜索中捕捉意外发现，Phase 4 独立成章。
...
为 Phase 2 的每个主题执行 3 个 query，**orchestrator 自己搜**（不 spawn agent）：
1. 用搜索工具（searxng/web_search_prime/WebSearch）执行 Phase 2 构造的 query
2. 从结果中筛选 URL（每个主题 3-5 个高相关 URL）
3. URL 自动去重
4. **URL 质量过滤**：跳过纯营销页面、缺少实质内容的首页、纯视频/图片页面。优先：学术论文、技术博客、官方文档、深度分析文章
...
1. 用 ctx_search(queries=["{主题关键词1}", "{关键词2}"]) 搜索索引
   - 每个 query 返回的结果不够深 → 换更具体的 query 重搜
2. 每个主题提取 3-5 条核心要点（必须含具体数据/人名/机构/结论/时间，不要泛泛而谈）
3. 如果索引中信息不足，用 WebSearch 补充（最多 {N/2} 次）
4. **深度要求**：每条要点必须回答"具体是什么"——不能只说"XXX很重要"，要说出"为什么重要、重要到什么程度、有什么数据支撑"
5. 标记 💡灵感
6. 按标准格式输出
...
1. **信息提取**：从每个 agent 原始输出中提取具体事实（数据、人名、机构、结论、时间）。去掉泛泛而谈的段落——如果去掉某段后不影响理解，说明那段没有信息量
2. **交叉验证**：同一事实被多个独立来源确认 → `[✅ A,B确认]`；单一来源的事实 → `[⚠️ 单源: 来源A]`；不同来源矛盾 → `[⚡ 矛盾: A说X, B说Y]`
3. **可信度排序**：一手数据 > 二手分析 > 观点/评论。标注每条信息的层级
4. **知识空白识别**：哪些重要问题搜索了但没找到答案？用 `⚠️ 知识空白:` 标注，不要用模糊语言掩盖
5. **灵感筛选**：💡意外发现按意外性排序，每个灵感说明为什么意外 + 可引出什么新问题
6. **撰写报告**

**报告结构**:
1. **背景与目标**（2-3句）
2. **核心发现**（每条 `[来源#N]`，含具体数据，标注可信度层级）
3. **方案对比**（仅对比型，表格，每格有数据支撑）
4. **💡 意外发现**（按类型：反常识/跨领域/被忽视/打破认知/未解问题）
5. **⚠️ 知识空白**（搜了但没找到答案的问题）
6. **结论与建议**（3-5条，每条有来源支撑）
7. **信息源列表**（编号 + URL + 可信度层级）

规则：中文 | 每条发现必须有来源 | 知识空白必须标注 | 不编造 | 去掉信息密度为零的段落
