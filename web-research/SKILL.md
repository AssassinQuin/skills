...
```
Bash('MCP_TOOL="${MCP_TOOL:-opencode}"; bash {skill_dir}/scripts/mcp-probe.sh --tool "$MCP_TOOL"')  → 缓存命中则跳过
Read('{skill_dir}/data/.mcp-cache-${MCP_TOOL:-opencode}.json')        → 获取 available/unavailable 列表
```

缓存由脚本维护（`claude mcp list` → hash → 变化时更新），模型不做工具探测调用。
