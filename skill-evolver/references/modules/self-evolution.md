...
| application | 主 agent 改写 SKILL.md | **扩展范围** | 改 SKILL.md + references/ + scripts/ |
| audit | opus 读新版 + .before | **同** | 已 commit，正常对比 |
| deployment | T_val 测试 | **meta 测试** | 测试流程可执行性 |
...
- 策略可影响 SKILL.md、references/、scripts/ 下的所有文件
- 不要破坏流程骨架（baseline→exploration→application→audit→deployment）
- 保持脚本驱动（phase-check、phase-start、quick-fix-check）
```

## Meta 测试设计

T_train/T_val 的测试 prompt 测"流程可执行性"：
...
```json
{
  "T_train": [
    ...
    {"id": "T3", "type": "Quick Fix", "prompt": "优化 neat-freak，痛点：触发词不匹配", "expect": "走 Quick Fix 路径，跳过 exploration"},
    {"id": "T4", "type": "回滚处理", "prompt": "审计 Score ≤ 基线的场景", "expect": "git reset HEAD~1 回到 CP-02"}
  ],
  "T_val": [
...
commit 包含 SKILL.md + 所有修改过的 references/ 和 scripts/ 文件。
