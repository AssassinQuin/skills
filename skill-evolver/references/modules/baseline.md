...
读取 `metrics.json` 和 `evolution-log.jsonl`，检测以下信号：

| 信号 | 检测方法 | 阈值 |
|------|---------|------|
| 补丁边际递减 | `avg_score_delta` 连续 2 轮 < 0.5 | 触发重构 |
| 版本轮次过多 | `total_rounds` >= 4 | 触发重构 |
| 文件膨胀 | `wc -l SKILL.md` > 200 行 | 触发重构 |
| 参考文件膨胀 | `ls references/**/*.md \| wc -l` > 10 | 触发重构 |
| 约束堆叠 | grep -c "^[0-9]" SKILL.md 的约束节 > 15 | 触发重构 |

**任一信号命中** → 输出诊断报告 + 推荐大重构（不走补丁路径），让用户选择：

```
检测到膨胀信号（{命中信号列表}）：
- 已有 {N} 轮补丁进化，边际收益 Δ={delta}
- SKILL.md {lines} 行 / {refs} 个参考文件 / {constraints} 条约束

推荐：大重构（跨文件去重合并 + 精简约束 + 流程重组）
[A] 大重构（推荐）
[B] 继续补丁进化（你确认方向）
```

用户选 A → 策略限定 S2(重组) + S5(范式转换) 的合成，排除 S1(精化) 等补丁策略。
用户选 B → 正常补丁流程，但记录 `override_bloat_detection` 到 evolution-log。
