...
- **2026-06-18 v8.1 增强**：基于实战复盘（进化某研究型 skill 暴露的问题），新增 §14 research-skill adaptation，补齐论文未覆盖的三类 skill 适配。

---

## 14. Research-Skill Adaptation（v8.1 新增，非论文原版）

**论文 §3.1 假设**：Domain-Skill Agent 拿 candidate skill + T_train 任务，真实执行，产出 (τ, y) 其中 y 是 binary reward。

**论文未覆盖**：研究型 skill（如 skill-search / web-research / code-review）的"任务"是评估一个外部对象，没有 binary 成功/失败，只有定性产出质量。文档型 skill（如 coding-rules）甚至无可执行任务。

### 14.1 Skill 类型分类

详见 [modules/skill-taxonomy.md](modules/skill-taxonomy.md)。三种类型：
...
### 14.2 与 Algorithm 1 的兼容性

**Algorithm 1 骨架不变**（Phase 1-4 + surgical patch + 9-check audit），只是 Phase 2-3 的 Explore 函数内部根据 skill 类型替换：

```
Explore(T_train, S_r, v_r, K):
  if skill_type == "execution":
    return K 个 Domain-Skill Agent 真实执行，y ∈ {0, 1}
  elif skill_type == "research":
    return K 个 Evaluation-Trace Agent 真实评估，y ∈ {A-F}
  elif skill_type == "document":
    return K 个 Reviewer Checkpoint 5-question，y ∈ [0, 1]
```

**Contrast 函数也相应适配**：
- 执行型：(τ+, τ-) = (y=1 轨迹, y=0 轨迹)
- 研究型：(τ+, τ-) = (高质量产出, 低质量产出)，对比 BEFORE
- 文档型：(τ+, τ-) = (5/5 通过, ≤2/5 通过)

### 14.3 反例（v8.0 → v8.1 修复的实战 bug）

**bug 1: 研究型 skill 硬套执行型 Algorithm 1**
- v8.0 做法：跳过 Domain-Skill Agent，用"上一版评估复盘"作为 failure signal
- 问题：这不是 (τ, y) 轨迹，是事后归因
- v8.1 修复：Phase 1 检测为研究型 → spawn Evaluation-Trace Agent 真实跑 v_{n+1} 评估一个目标对象，对比 v_n 跑同一任务

**bug 2: T_train = T_val 过拟合**
- v8.0 做法：进化 + 验证都用同一评估对象
- 问题：过拟合到单一场景，audit Check 6/7 暴露训练案例渗透（critical violations）
- v8.1 修复：Phase 1 Gate 3 强制 T_train ≠ T_val，研究型 skill 用 3+2 拆分（3 个训练对象，2 个 held-out 不同类型对象）

**bug 3: Audit 只在最后做**
- v8.0 做法：早轮 patch 跳过 audit，最后一轮才做
- 问题：早轮带病前进（多个过拟合 violation），最后才发现
- v8.1 修复：Phase 3 Gate 2 强制每轮 audit，reject 不允许跨 r

### 14.4 与论文的关系（重要）

§14 **不是论文的扩展**，是基于实战的**论文未覆盖场景的工程适配**。论文 §5 limitations 提到：
> "Our evaluation focuses on skills with executable tasks and verifiable outputs. Extending to skills with subjective or multi-dimensional success criteria is left to future work."

v8.1 的 research-skill adaptation 正是这个 "future work" 的实现。但严格意义上，**论文 Algorithm 1 只保证执行型 skill**。研究型 / 文档型 是 v8.1 的工程扩展，**不享有论文实验数据的支持**。

如果发现 v8.1 research-track 在某些研究型 skill 上失效，应该回归论文原版（即降级为 binary reward，让用户手动判定）。
