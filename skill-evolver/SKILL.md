---
name: skill-evolver
version: "6.5"
description: >
  Skill 自进化框架。基于 SkillEvolver 论文(arXiv:2605.10500)实现。
  命令驱动 + 脚本强制 + 渐进披露 + 双路径 + 部署接地审计。
  v6: 范式转换——文档式→命令驱动，脚本强制调用，轻量验证模式。
  Trigger: "进化 X skill", "审计 X", "评估 X 质量", "进化 skill-evolver",
  "优化所有 skills", "查看进化历史", "重写 skill", "优化 X，痛点 Y", "优化 X skill",
  "验证 X skill".
allowed-tools:
  - Read
  - Write
  - Edit
  - Bash
  - Glob
  - Grep
  - Agent
  - AskUserQuestion
user-invocable: true
---

# Skill Evolver v6.3

## 决策入口

| 意图 | 模式 | 路径 |
|------|------|------|
| "进化 X" / "优化 X" / "优化 X，痛点 Y" | A | baseline → exploration → application → audit → deployment（痛点路径由 quick-fix-check 自动判定） |
| "审计 X" | B | audit 独立执行 |
| "评估 X 质量" / "验证 X" / 编辑 SKILL.md 后 | C | baseline（评估+轨迹）或 `skill-validate` 轻量检查 |
| "进化 skill-evolver" | D | A + [self-evolution.md](references/modules/self-evolution.md) 适配 |
| "优化所有 skills" | E | baseline 筛选 → 逐个 Mode A |
| "查看进化历史" | — | 读 `.evolve/evolution-log.jsonl` |
| "重写 skill" | — | 直接 skill-creator |

痛点：`pp-create` → `.evolve/pain-points.jsonl` → `quick-fix-check` 判定路径。

## 价值锚定

> **宁可产出低分但诚实的进化结果，也不要高分但编造的通过率。** 诚实的 UNVERIFIED 比虚假的 4/4 更有价值。诚实的「本轮未能验证」比敷衍的「全部通过」更有价值。

当面临「诚实 → 低分」vs「编造 → 高分」的冲突时，选择诚实。

## 流程（命令驱动）

```
baseline → [quick-fix] → exploration → application → audit → deployment
              ↓ skip          ↑                            │
              └──────→ application ──→ audit → deployment ←┘
                                           ↑
                              └── r < R 继续 ──── 退化回滚
```

**Quick Fix 路径**：跳过 exploration，但 **audit 和 deployment 不可跳过**。Quick Fix 只省策略探索，不省质量验证。

**迭代上限**：baseline→deployment 最多循环 3 轮（R ≤ 3）。3 轮后仍有未通过项，在诚实边界中标注薄弱维度，交付当前最优版本。单个 Phase 回滚 ≥ 2 次 → 终止本轮。

### Phase 入口（强制 Bash 命令，不可跳过）

每个 Phase 开始时，**必须按顺序执行以下 Bash 命令**：

```bash
# 1. 前置条件检查（失败 = 停止）
source /Users/ganjie/.claude/skills/skill-evolver/scripts/evolve.sh && phase-check <phase> <skill_dir>
# 2. 开始标记
source /Users/ganjie/.claude/skills/skill-evolver/scripts/evolve.sh && phase-start <phase> <skill_dir>
```

然后：`Read references/modules/<phase>.md` → 按模块步骤执行。

**脚本等价操作必须用脚本**（禁止手动替代）：

| 操作 | 脚本命令 | 必用 Phase |
|------|---------|-----------|
| 评分 | `score D1 D2 D3 D4 D5` | baseline, audit, deployment |
| 痛点创建 | `pp-create <dir> <id> <desc> <symptom> <source> <round>` | baseline |
| 痛点解决 | `pp-resolve <dir> <id> <by>` | audit, deployment |
| 痛点回归 | `pp-regress <dir> <id>` | audit |
| 指标更新 | `metrics-update <dir> <round> <strategy> <before> <after> <d1>..<d5> <t_train> <t_val>` | deployment |
| Git 分支 | `git-setup <name> <dir>` | baseline |
| Git 提交 | `git-checkpoint <msg> <dir>` | 各 Phase 末尾 |
| Quick Fix | `quick-fix-check <dir>` | baseline |
| 绕过检测 | `silent-bypass-check <dir>` | audit |
| 质量验证 | `verify-metrics <dir>` | audit |
| 轻量检查 | `skill-validate <dir>` | Mode C / 编辑后 |
| 保存快照 | `snapshot-save <dir>` | baseline (BEFORE 副本) |
| 保存审计 | `audit-save <dir> <round>` | audit (审计报告) |

### 确认点（AskUserQuestion 强制）

每个 Phase 完成后，**必须**调用 `AskUserQuestion` 展示结果并等待用户确认。未收到确认禁止进入下一 Phase。

| Phase | 展示内容 | **本轮局限**（必须展示） | 检查点意义 | 确认后 |
|-------|---------|----------------------|-----------|--------|
| baseline | 基线评分 + 痛点表 + Quick Fix 判定 | trace 覆盖度、信息盲区 | 基线决定了进化方向，方向错了后面全白费 | → exploration |
| exploration | 候选策略排名 + 对比分析 | 策略覆盖盲区 | 策略选错 = 整轮白费，在这里拦截成本最低 | → application |
| application | 修改文件列表 + diff 摘要 | 未覆盖的痛点、超出范围的修改 | 确认改动是否精确对应痛点，防止改多了或改偏了 | → audit |
| audit | 审计报告 + **进化效果对比** | 未验证维度、依赖自律的约束 | audit 是最终质量门，通过后直接进入部署，必须在此拦截问题 | → deployment |
| deployment | 最终评分 + **痛点回归** + T_val | UNVERIFIED 项、未执行的测试 | deployment 是上线门控，回归问题不在此拦截就会带到生产 | → merge |

**"本轮局限"**：每个 Phase 必须列出本轮未做到、未验证、或依赖 agent 自律而非结构保障的事项。禁止写"无"——如果真的没有任何局限，写"已全部覆盖"并附证据。

### 进化效果对比（audit 阶段展示）

除审计评分外，必须展示：

```
## 进化效果对比
### 问题解决
| 痛点 | 进化前 | 进化后 | 改善 |
|------|--------|--------|------|
| PP-xxx | 描述问题 | 如何解决 | 量化效果 |

### 量化对比
| 指标 | Before | After | Δ |
|------|--------|-------|---|
| Score | X.X | Y.Y | +Z.Z |
| 约束数 | N | M | -K |
| 行数 | L1 | L2 | -L |
| 痛点 open | O1 | O2 | -P |
```

### 痛点回归测试（deployment 阶段强制）

deployment 阶段必须执行：
1. 读取 `{skill}/.evolve/pain-points.jsonl` 所有 `resolved` 状态的痛点
2. 对每个 resolved 痛点验证其修复是否仍然有效（检查 SKILL.md 中对应内容）
3. 任何回归 → `pp-regress` + 回滚到 CP-03
4. 全部通过 → 继续部署

### 诚实边界（每轮进化必须输出）

进化完成时，**必须**在 deployment 确认点中包含以下诚实声明：

```
## 诚实边界
### 本轮未验证
- {列出 T_train/T_val 中未实际执行的测试，标注 UNVERIFIED}

### 本轮未覆盖
- {列出本轮策略未涉及的痛点或维度}

### 结构性局限
- {列出依赖 agent 自律而非结构保障的约束（如"子agent必须spawn"仍靠文字声明）}

### 信息盲区
- {列出 trace 不足、缺乏实证数据的领域}
```

禁止留空或写"无"。至少列出 1 条本轮局限。

## 子 Agent

| 任务 | subagent_type | model | 模板 |
|------|--------------|-------|------|
| 策略探索 S0-S6 | `evolver-explorer` | sonnet | [templates.md#Explorer](references/modules/prompts/templates.md) |
| 独立审计 | `evolver-auditor` | opus | [templates.md#Auditor](references/modules/prompts/templates.md) |
| T_val 验证 | `evolver-auditor` | opus | [templates.md#Deployer](references/modules/prompts/templates.md) |
| D5 基线测试 | `evolver-explorer` | sonnet | 内联（baseline 模块内） |

R5.1：`Agent()` 必须同时传 `subagent_type` + `model`。Agent 是 deferred 工具，调用前需 `ToolSearch select:Agent` 加载 schema。

## 评分 / 痛点

详见 [baseline.md](references/modules/baseline.md) 评分体系 + 痛点章节。失败模式见 [failure-modes.md](references/failure-modes.md)。脚本命令：`score D1 D2 D3 D4 D5`、`pp-create`、`pp-resolve`、`pp-regress`。

## Git

分支：`evolve/{skill}/YYYYMMDD`。BEFORE 副本保存到 `{skill}/.evolve/snapshots/{skill-name}-{timestamp}.md`。

| 时机 | Checkpoint |
|------|-----------|
| baseline | CP-01: `git-setup <name> <dir>` |
| exploration | CP-02: `git-checkpoint <msg> <dir>` |
| application | CP-03: `git-checkpoint <msg> <dir>` |
| deployment | CP-04: `git-checkpoint <msg> <dir>` |
| audit 失败 | `git reset HEAD~1` → CP-02 |
| 部署退化 | `git revert HEAD` → CP-03 |
| 收工 | `git checkout main && git merge` |

## 日志

所有日志位于 `{skill}/.evolve/`。关键文件：`evolution-log.jsonl`、`metrics.json`、`pain-points.jsonl`、`deployment-traces.jsonl`。

**Deployment-Grounded Learning**：每次 baseline 启动时必须检查 `deployment-traces.jsonl` 是否有上次部署后的使用反馈。空文件 = D5 维度无实证数据，必须在诚实边界中声明。

## 约束（8 条 + 绝对不做）

### 绝对不做（违反 = 进化结果无效）

1. **不编造通过率** — T_train/T_val 未执行就填数字
2. **不跳过审计** — Quick Fix 也不跳过 audit 和 deployment
3. **不跳过痛点回归** — deployment 必须验证 resolved 痛点
4. **不在未执行时填结果** — 子 agent 未 spawn 则标记 UNVERIFIED，禁止填通过率
5. **不忽略历史痛点** — 新改动导致已解决痛点回归时必须记录

**Mode C: skill-validate 检查清单**（`source evolve.sh && skill-validate <dir>`）

| # | 检查项 | 通过标准 |
|---|--------|---------|
| 1 | SKILL.md 存在 | 文件可读 |
| 2 | Frontmatter 必需字段 | 有 name + description |
| 3 | 补丁饱和 | metrics.json total_rounds < 5 |
| 4 | 参考文件数 | references/ ≤ 10 个 .md |
| 5 | Silent-bypass 信号 | 无 WARN 信号 |
| 6 | 约束数量 | ≤ 15 条 |

1. **命令驱动** — Phase 入口必须通过 Bash 执行 `phase-check` + `phase-start`；Phase 结尾必须 `AskUserQuestion`
2. **脚本等价优先** — 有脚本命令的操作禁止手动替代（评分、痛点、Git、指标、快照、审计保存）
3. **路径锚定** — 子 agent 绝对路径 + ls 验证
4. **审计隔离** — BEFORE 副本（含目录清单 manifest）+ 全新上下文 opus + 审计子 agent 可读取 skill 目录下所有非 .evolve/ 文件（README.md、agents/、scripts/ 等）+ 禁止引用 training traces + **Quick Fix 也必须独立审计**
5. **写入集中** — 主 agent 写，子 agent 只读
6. **完整改写** — 不打补丁；T_train/T_val 隔离
7. **R ratchet** — 只保留有改进的版本；退化回滚
8. **补丁饱和检测** — `total_rounds >= 5` 时触发精简建议（删补丁式说明，保留框架结构），不强制重构
9. **测试真实执行** — T_train/T_val 必须由子 agent 实际执行并返回结果，禁止在未执行时填写通过率。日志中 T_train/T_val 通过率必须有子 agent 的实际输出摘要作为证据（引用子 agent 返回的关键行），不可只填数字
10. **BEFORE 可追溯** — 副本必须保存到 `.evolve/snapshots/`（`snapshot-save`），禁止用 `/tmp/`。审计结束后不删除快照

已合并到约束/脚本的旧约束：渐进披露(→模块按需加载)、Token上限(→baseline.md)、子agent不重试(→exploration.md)、重构优先补丁(→约束8)、silent-bypass(→脚本命令)、Git锚定(→git-setup参数)、用户确认(→确认点表格)、workspace隔离(→约束4)、行数膨胀检测(→补丁饱和检测)
