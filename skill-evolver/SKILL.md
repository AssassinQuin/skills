...
| 维度 | 权重 | 审计检查项 | 评分锚点 |
|------|------|-----------|----------|
| D1 Frontmatter | 10% | Framing + X-ref | 元数据完整、引用可达 |
| D2 Workflow | 20% | Coverage + Silent-bypass | 场景覆盖、步骤不可跳过 |
| D3 Boundary | 15% | Script-bloat + Shape-bake | 无膨胀、无过度硬化 |
| D4 Precision | 20% | Literals + Untraceable + Under-abstraction | 字面正确、无模糊、无重复 |
| D5 Empirical | 35% | Overfit (T_val) | held-out 泛化验证 |

**公式**：`Score = D1×0.10 + D2×0.20 + D3×0.15 + D4×0.20 + D5×0.35`（结果 0-10）

**baseline 阶段**：D1-D4 静态分析，D5 = (T_train_pass / T_train_total) × 10
**audit 阶段**：独立 opus 按 5 维重新评分（每个维度对应 2-3 个检查项），产出 D1-D5 分数
...
audit ─────── Read references/modules/audit.md
  ...
  │            独立 opus 5 维评分 + 标记验证 + T_val Overfit 检查
...
| audit+deployment | 审计 Score > 基线 且 无维度 < 5 且 diff≤200 行 | Score ≤ 基线 或 有维度 < 5 或 diff>200 行 |
...
| audit Score ≤ 基线 | `git reset HEAD~1` | 回到 CP-02 |
