...
- design.md §5.2 + §5
- [`codebase-memory-mcp.md`](../coder/modules/codebase-memory-mcp/SKILL.md)（MCP 触达点 1）
- v7.3 来源：mattpocock `improve-codebase-architecture` skill

---

## S.U.P.E.R 模块健康评估（v7.2 合并自 phase-1-super-check）

> **加载时机**：Phase 1 explorer 返回 modules 清单后，orchestrator 评分时。
> v7.2：原 `phase-1-super-check` module 合并到本 skill 末尾。
...
- 新改动让模块从 🟢 → 🟡/🔴 → **强制 Phase 5 深度审计**（coder-verify）
...
# U 单向流：用 mcp__codebase-memory-mcp__trace_path 检测循环

# E 无硬编码
grep -rE "(localhost|127\.0\.0\.1|password|secret|api_key)" internal/ --include="*.go"

# R 可替换：用 get_architecture 看 incoming/outgoing 比
```
